...
class TwoFactorService
{
    ...
    public function generateAndSend(int $userId, string $email, string $name): array
    {
        ...
        // Limpia códigos expirados o ya usados
        $this->db->prepare('
            DELETE FROM two_factor_codes
            WHERE user_id = ? AND (expires_at < NOW() OR used_at IS NOT NULL)
        ')->execute([$userId]);

        // Comprueba cooldown
        $stmt = $this->db->prepare('
            SELECT id, created_at FROM two_factor_codes
        ...
        if ($existing) {
            ...
            if ($secondsSince < $cooldown) {
                ...
                return [
                    'success'   => false,
                    'error'     => "Espera {$remaining} segundo(s) antes de solicitar otro código.",
                    'cooldown'  => true,
                    'remaining' => $remaining,
                ];
            }
            // Más de cooldown segundos → borra el anterior
            $this->db->prepare('
                DELETE FROM two_factor_codes WHERE user_id = ? AND used_at IS NULL
            ')->execute([$userId]);
        }
        ...
        $this->db->prepare('
            INSERT INTO two_factor_codes (user_id, code_hash, expires_at, request_ip)
            VALUES (?, ?, ?, ?)
        ')->execute([$userId, $codeHash, $expiresAt, $ip]);

        $sent = $this->sendCodeEmail($email, $name, $code);

        if (!$sent) {
            // Si el email falla, borra el código para no bloquear futuros intentos
            $this->db->prepare('
                DELETE FROM two_factor_codes
    ...
    public function verify(int $userId, string $code): array
    {
        ...
        if (!hash_equals($record['code_hash'], hash('sha256', $code))) {
            ...
            if ($attempts >= $maxAttempts) {
                ...
                $this->db->prepare('
                    UPDATE two_factor_codes SET failed_attempts = ?, blocked_until = ? WHERE id = ?
                ')->execute([$attempts, $blockedUntil, $record['id']]);
                return ['success' => false, 'error' => 'Demasiados intentos fallidos. Espera 15 minutos.', 'blocked' => true];
            }

            $this->db->prepare('
                UPDATE two_factor_codes SET failed_attempts = ? WHERE id = ?
            ')->execute([$attempts, $record['id']]);

            $remaining = $maxAttempts - $attempts;
            return ['success' => false, 'error' => "Código incorrecto. Te quedan {$remaining} intento(s)."];
        }
    ...
    {
        $subject = 'Tu código de verificación — PrimeLux SmartShop';
        $html    = $this->buildEmailTemplate($name, $code);
        return $this->mailer->send($to, $name, $subject, $html);
    }
    ...
    private function buildEmailTemplate(string $name, string $code): string
    {
        ...
        $digits = implode('', array_map(
            fn($d) => "<span style='display:inline-block;width:40px;height:48px;line-height:48px;
                       text-align:center;font-size:28px;font-weight:700;color:#FFFFFF;
                       background:#1F2937;border-radius:8px;margin:0 4px;
                       border:1px solid #374151;'>{$d}</span>",
            str_split($code)
        ));
  ...
  <table width="100%" cellpadding="0" cellspacing="0" border="0">
    <tr><td align="center" style="padding:40px 20px;">
      <table width="480" cellpadding="0" cellspacing="0" border="0"
             style="background:#1F2937;border-radius:16px;overflow:hidden;max-width:480px;">
        ...
        <tr><td style="padding:32px;">
          <p style="color:#9CA3AF;font-size:15px;margin:0 0 6px;">
          ...
          <p style="color:#6B7280;font-size:13px;text-align:center;margin:0 0 24px;">
            Expira en <strong style="color:#F59E0B;">{$minutes} minutos</strong>.
          </p>
          <div style="background:#111827;border-radius:8px;padding:16px;border:1px solid #374151;">
            <p style="color:#6B7280;font-size:12px;margin:0;">
              Si no has iniciado sesión en PrimeLux SmartShop, ignora este mensaje.
            </p>
          </div>
        </td></tr>
